...
JNIEXPORT jobjectArray JNICALL Java_x_leon_DarknetDao_inference
    ...
    std::string image_name  = env->GetStringUTFChars(image_name_j, JNI_FALSE);
    // image im = readRawImageData(image_name);
    image im = load_image_stb(&image_name[0], 3);
    ...
    if (NULL != c) {
        mid = (*env).GetMethodID(c, "<init>", "(FFFFFLjava/lang/String;)V");
        // mid = env->GetMethodID(c, "<init>","Lx/leon/Result;(IIIIFLjava/lang/String;)V");
        if (NULL != mid) {
            for (int i = 0; i < results.size(); ++i) {
                Result r = results[i];
                jfloat l = ((float)r.left)/im.w;
                jfloat w = ((float)(r.right-r.left))/im.w;
                jfloat t = ((float)r.top)/im.h;
                jfloat h = ((float)(r.bot-r.top))/im.h;
                jstring jlabel = env->NewStringUTF(r.label.c_str());
                jfloat conf = r.confidence;

                jobject jObj = (*env).NewObject(c, mid, l, t, w, h, conf, jlabel);
                (*env).SetObjectArrayElement(returnArr, i, jObj);
                LOGI("JNI: %s %f %f %f %f %f" ,r.label.c_str() ,  r.left , r.top , r.right , r.bot , r.confidence);
...
JNIEXPORT jboolean JNICALL Java_x_leon_DarknetDao_load
    ...
    std::string name_list_file = env->GetStringUTFChars(name_list_file_j,  JNI_FALSE);
    std::string cfgfile = env->GetStringUTFChars(cfgfile_j,  JNI_FALSE);
    std::string weightfile = env->GetStringUTFChars(weightfile_j, JNI_FALSE);

    // api = new DarknetAPI(&name_list_file[0u], &cfgfile[0u], &weightfile[0u]);
    api = new DarknetAPI(&name_list_file[0], &cfgfile[0], &weightfile[0]);
