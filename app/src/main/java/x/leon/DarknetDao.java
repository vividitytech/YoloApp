...
public class DarknetDao {
    ...
    public static DarknetDao getInstance(Context context) {
        synchronized (DarknetDao.class) {
            ...
            if (api == null) {
                copyFiles(context);
                api = new DarknetDao();
                boolean rst = api.load(PATH+"coco.names", PATH+"yolov3tiny.cfg",
    ...
    private static void copyFiles(Context context){
        ...
        new File(PATH).mkdirs();
        UtilFile.CopyDataToSdcard(context,"coco.names",PATH+"coco.names");
        UtilFile.CopyDataToSdcard(context,"yolov3-tiny.weights",PATH+"yolov3-tiny.weights");
        UtilFile.CopyDataToSdcard(context,"yolov3tiny.cfg",PATH+"yolov3tiny.cfg");
    }
